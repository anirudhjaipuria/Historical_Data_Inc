...
def is_invalid_trendline_vectorized(opens, highs, lows, closes, slope, y_intercept, n, start_idx, end_idx):
    """
    Vectorized check for invalid downward trendlines.
...
def calc_TL_Down_Resistance(df, min_gap, adjacent_candles, exclude_end_points):
    """
    Full calculation of downward resistance trendlines.
    ...
    for i in range(1, 8):
        df[f'TL_D_Resistance_{i}_Start_Date'] = pd.NaT
        df[f'TL_D_Resistance_{i}_End_Date'] = pd.NaT
        df[f'TL_D_Resistance_{i}_Start_Price'] = 0
        df[f'TL_D_Resistance_{i}_End_Price'] = 0
        df[f'TL_D_Resistance_{i}_next_file_Price_projection'] = 0
        df[f'TL_D_Resistance_{i}_Date_Gap'] = 0
    ...
    for i in range(1, 8):
        ...
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_Date_Gap'] = 0
    ...
    # Filtering: keep the highest (closest to price action) when clustered
    filtered_trendlines = []
    for tl in trendlines:
        ...
        if is_adjacent:
            all_higher = all(start_high >= atl[1] and end_high >= atl[3] for atl in adjacent_trendlines)
            if all_higher:
    ...
    if sorted_trendlines:
        for idx, tl in enumerate(sorted_trendlines, 1):
            ...
            df.loc[last_row_idx, f'TL_D_Resistance_{idx}_Date_Gap'] = date_gap

            if date_gap != 0:
                next_x = n
                projection = slope * next_x + y_intercept
                df.loc[last_row_idx, f'TL_D_Resistance_{idx}_next_file_Price_projection'] = projection

    return df, trendlines  # all valid trendlines (sorted) as state


def update_TL_Down_Resistance(df, previous_all_valid, min_gap, adjacent_candles, exclude_end_points):
    """
    Incremental update for downward resistance trendlines.
    """
    df['date'] = pd.to_datetime(df['date'], utc=True).dt.tz_localize(None)
    ...
    for i in range(1, 8):
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_Start_Date'] = first_date
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_End_Date'] = last_date
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_Start_Price'] = 0
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_End_Price'] = 0
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_next_file_Price_projection'] = 0
        df.loc[last_row_idx, f'TL_D_Resistance_{i}_Date_Gap'] = 0
