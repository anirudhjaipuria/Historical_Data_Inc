...
def HBearDivg_analysis(ohlc: pd.DataFrame, Candle_Tol: float, MACD_tol: float) -> pd.DataFrame:
    ...
    HBearD_diffY_price_1 = np.zeros(n)
    HBearD_diffX_1 = np.zeros(n)
    ...
    HBearD_diffY_price_2 = np.zeros(n)
    HBearD_diffX_2 = np.zeros(n)
    ...
    HBearD_diffY_price_pos = np.zeros(n)
    HBearD_diffX_pos = np.zeros(n)

    # Initialize for gen
    HBearD_diffY_gen = np.zeros(n)
    HBearD_diffX_gen = np.zeros(n)
    ...
    if len(valid_indices_1) > 0:
        ...
        HBearD_diffY_price_1[valid_indices_1[bearish_divergence_1]] = price_diff[bearish_divergence_1]
        HBearD_diffX_1[valid_indices_1[bearish_divergence_1]] = valid_indices_1[bearish_divergence_1] - last_nonzero_indices_1[bearish_divergence_1]
    ...
    if len(valid_indices_2) > 0:
        ...
        HBearD_diffY_price_2[valid_indices_2[bearish_divergence_2]] = price_diff_price[bearish_divergence_2]
        HBearD_diffX_2[valid_indices_2[bearish_divergence_2]] = valid_indices_2[bearish_divergence_2] - last_nonzero_indices_2[bearish_divergence_2]
    ...
    if len(valid_indices_3) > 0:
        ...
        # Store diffY_price and diffX for price slope
        HBearD_diffY_price_pos[valid_indices_3[bearish_divergence_3]] = price_diff_price[bearish_divergence_3]
        HBearD_diffX_pos[valid_indices_3[bearish_divergence_3]] = valid_indices_3[bearish_divergence_3] - last_nonzero_indices_3[bearish_divergence_3]
    ...
    for idx in valid_indices_gen:
        if HBearD_1[idx] == 1 or (HBearD_1[idx] == 1 and HBearD_2[idx] == 1):
            ...
            # Set diff for gen
            HBearD_diffY_gen[idx] = HBearD_diffY_price_1[idx]
            HBearD_diffX_gen[idx] = HBearD_diffX_1[idx]
        elif HBearD_2[idx] == 1:
            HBearD_gen[idx] = 1
            ...
            # Set diff for gen
            HBearD_diffY_gen[idx] = HBearD_diffY_price_2[idx]
            HBearD_diffX_gen[idx] = HBearD_diffX_2[idx]
    ...
    macd_high_rolling_max = pd.Series(macd_hist).rolling(window=bars, min_periods=1).max()
    macd_low_rolling_min = pd.Series(macd_hist).rolling(window=bars, min_periods=1).min()
    Yaxis_macd_series = macd_high_rolling_max - macd_low_rolling_min

    # Compute angles for gen
    HBearD_gen_Slope = np.zeros(n)
    gen_indices = np.where(HBearD_gen == 1)[0]
    for idx in gen_indices:
        Yaxis = Yaxis_series.iloc[idx]
        if Yaxis == 0:
            Yaxis = 1e-6
        diffY = HBearD_diffY_gen[idx]
        diffX = HBearD_diffX_gen[idx]
        abs_diffY = abs(diffY)
        ...
        HBearD_gen_Slope[idx] = angle

    # Compute angles for pos_MACD
    HBearD_pos_MACD_Slope = np.zeros(n)
    pos_indices = np.where(HBearD_pos_MACD == 1)[0]
    for idx in pos_indices:
        Yaxis = Yaxis_series.iloc[idx]
        if Yaxis == 0:
            Yaxis = 1e-6
        diffY = HBearD_diffY_price_pos[idx]
        diffX = HBearD_diffX_pos[idx]
        abs_diffY = abs(diffY)
        if diffX == 0 or abs_diffY == 0:
            angle = 0.0
        else:
            ...
            normalised_slope = (height / diffY_to_Yaxis) / diffX
            angle = round(math.atan(normalised_slope) * 180 / math.pi, 2)
        HBearD_pos_MACD_Slope[idx] = angle

    # Compute MACD angles for gen
    HBearD_gen_MACD_Slope = np.zeros(n)
    for idx in gen_indices:
        Yaxis_macd = Yaxis_macd_series.iloc[idx]
        if Yaxis_macd == 0:
            Yaxis_macd = 1e-6
        diffY_macd = HBearD_Higher_High_MACD_gen[idx] - HBearD_Lower_High_MACD_gen[idx]
        ...
        if diffX == 0 or abs_diffY_macd == 0:
            ...
            angle = round(math.atan(normalised_slope) * 180 / math.pi, 2)
        HBearD_gen_MACD_Slope[idx] = angle

    # Compute MACD angles for pos_MACD
    HBearD_pos_MACD_MACD_Slope = np.zeros(n)
    for idx in pos_indices:
        Yaxis_macd = Yaxis_macd_series.iloc[idx]
        if Yaxis_macd == 0:
            Yaxis_macd = 1e-6
        diffY_macd = HBearD_Higher_High_MACD_pos_MACD[idx] - HBearD_Lower_High_MACD_pos_MACD[idx]
        abs_diffY_macd = abs(diffY_macd)
        diffX = HBearD_diffX_pos[idx]
        if diffX == 0 or abs_diffY_macd == 0:
            angle = 0.0
            ...
            normalised_slope = (height_macd / diffY_to_Yaxis) / diffX
            angle = round(math.atan(normalised_slope) * 180 / math.pi, 2)
        HBearD_pos_MACD_MACD_Slope[idx] = angle
    ...
    ohlc['HBearD_gen_Slope'] = HBearD_gen_Slope
    ohlc['HBearD_gen_MACD_Slope'] = HBearD_gen_MACD_Slope
    ...
    ohlc['HBearD_pos_MACD_Slope'] = HBearD_pos_MACD_Slope
    ohlc['HBearD_pos_MACD_MACD_Slope'] = HBearD_pos_MACD_MACD_Slope

    return ohlc[['HBearD_gen', 'HBearD_pos_MACD', 'HBearD_gen_Slope', 'HBearD_pos_MACD_Slope', 'HBearD_gen_MACD_Slope', 'HBearD_pos_MACD_MACD_Slope']]
